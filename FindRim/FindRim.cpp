// FindRim.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
	...
	cv::Mat srcMat=imread("E:\\课程\\大二下\\数字图像处理\\rim.png",0);
	cv::Mat Result;
	cv::Mat	BinaryMat;
	cv::Mat	OpenMat;
	...
	GaussianBlur(BinaryMat, GaussFilterMat, Size(3, 3), 6, 5, BORDER_DEFAULT);//高斯滤波

	vector<vector<Point>> contours;
	findContours(GaussFilterMat, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
